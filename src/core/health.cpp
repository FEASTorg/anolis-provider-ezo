...
ProviderHealth make_provider_health(const runtime::RuntimeState &state) {
  ...
  health.set_state(degraded ? ProviderHealth::STATE_DEGRADED
  ...
  (*health.mutable_metrics())["call_success_total"] =
      std::to_string(call_success_total);
  (*health.mutable_metrics())["call_failure_total"] =
      std::to_string(call_failure_total);
  ...
  if (!state.i2c_metrics.last_error.empty()) {
    (*health.mutable_metrics())["i2c_last_error"] =
        state.i2c_metrics.last_error;
  }

  for (const auto &excluded : state.excluded_devices) {
    (*health.mutable_metrics())["excluded_reason." + excluded.spec.id] =
        excluded.reason;
  }
...
std::vector<DeviceHealth> make_device_health(const runtime::RuntimeState &state,
  ...
  for (const auto &device : state.active_devices) {
    ...
    (*health.mutable_metrics())["address"] =
        format_i2c_address(device.spec.address);
    ...
    if (!device.startup_product_code.empty()) {
      (*health.mutable_metrics())["startup_product_code"] =
          device.startup_product_code;
    }
    if (!device.startup_firmware_version.empty()) {
      (*health.mutable_metrics())["startup_firmware"] =
    ...
    if (device.sample.has_sample) {
      ...
      (*health.mutable_metrics())["sample_age_ms"] =
          std::to_string(age_ms.count());
      (*health.mutable_metrics())["sample_stale_after_ms"] =
          std::to_string(stale_after);
      *health.mutable_last_seen() =
          to_proto_timestamp(device.sample.sampled_at);
    ...
    if (device.has_last_call) {
      (*health.mutable_metrics())["last_call_function"] =
          device.last_call_function;
      (*health.mutable_metrics())["last_call_ok"] =
          device.last_call_ok ? "true" : "false";
      const auto last_call_age_ms =
          std::chrono::duration_cast<std::chrono::milliseconds>(
              now - device.last_call_at);
      (*health.mutable_metrics())["last_call_age_ms"] =
          std::to_string(last_call_age_ms.count());
  ...
  if (include_excluded) {
    for (const auto &excluded : state.excluded_devices) {
      ...
      (*health.mutable_metrics())["address"] =
          format_i2c_address(excluded.spec.address);
      devices.push_back(std::move(health));
    }
  }
...
void populate_wait_ready(const runtime::RuntimeState &state,
  ...
  (*out.mutable_diagnostics())["excluded_device_count"] =
      std::to_string(state.excluded_devices.size());
  (*out.mutable_diagnostics())["call_success_total"] =
      std::to_string(call_success_total);
  (*out.mutable_diagnostics())["call_failure_total"] =
      std::to_string(call_failure_total);
  (*out.mutable_diagnostics())["provider_version"] =
      ANOLIS_PROVIDER_EZO_VERSION;
