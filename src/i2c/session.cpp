...
Status NoopSession::write_then_read(uint8_t, const uint8_t *, size_t, uint8_t *,
                                    size_t, size_t *rx_received) {
  if (!opened_) {
...
LinuxSession::LinuxSession(std::string bus_path, int timeout_ms,
                           int retry_count)
    : bus_path_(std::move(bus_path)), timeout_ms_(std::max(timeout_ms, 1)),
...
Status LinuxSession::open() {
  ...
  if (fd_ < 0) {
    return make_status(StatusCode::Unavailable, "failed to open " + bus_path_ +
                                                    ": " +
                                                    std::strerror(errno));
  }
...
Status LinuxSession::write_then_read(uint8_t address, const uint8_t *tx_data,
  ...
  if (tx_len > 0) {
    ...
    msgs[msg_count].buf =
        const_cast<__u8 *>(reinterpret_cast<const __u8 *>(tx_data));
    ++msg_count;
  }
  ...
  return make_status(StatusCode::Unavailable, "I2C_RDWR failed on " +
                                                  bus_path_ + ": " +
                                                  std::strerror(errno));
#else
  (void)address;
