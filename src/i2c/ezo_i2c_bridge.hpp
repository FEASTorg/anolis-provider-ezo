 ...
 * @brief Bridge from the provider's `ISession` abstraction into the EZO C transport API.
 */
...
/**
 * @brief Bound EZO device handle backed by a provider-managed session transport.
 */
...
/**
 * @brief Initialize an EZO C device handle that forwards transport calls through `ISession`.
 *
